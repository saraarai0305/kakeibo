...
window.addEventListener("load", () => setTimeout(async () => {
  ...
  try {
    ...
    if (!document.querySelector('[data-v2-work-import]') || !document.querySelector('[data-v2-work-import-save]')) throw new Error("UI smoke: legacy work import entry");
    const legacyImportCount = document.querySelectorAll('[data-v2-work-import]').length;
    tap('[data-v2-work-import]', "select existing work");
    tap('[data-v2-work-import-save]', "register existing work");
    await pause(100);
    if (document.querySelectorAll('.an-work-item').length < 1) throw new Error("UI smoke: registered existing work group");
    if (document.querySelectorAll('[data-v2-work-import]').length !== legacyImportCount - 1) throw new Error("UI smoke: registered work removed from candidates");
