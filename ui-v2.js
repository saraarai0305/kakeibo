...
(() => {
  ...
  function top(title, settings = page !== "settings"){
    return `<header class="v2-top">${stack.length ? `<button class="v2-back" data-v2-back>${icon("back")}戻る</button>` : `<span class="v2-top-spacer"></span>`}<h1>${esc2(title)}</h1>${settings ? `<button class="v2-top-action" data-v2-go="settings">${icon("settings")}</button>` : `<span class="v2-top-spacer"></span>`}</header><div class="v2-issue"><span>${issueDate()}</span><span>LIFE NOTE</span></div>`;
  }
