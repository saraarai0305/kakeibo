...
(() => {
  ...
  const recordMoney = value => {
    const amount = Number(value) || 0;
    const negative = amount < 0;
    return `<b class="v2-money-value ${negative ? "v2-negative" : ""}"><span class="v2-money-sign">${negative ? "−" : ""}</span><span class="v2-money-yen">￥</span><span class="v2-money-digits">${Math.abs(amount).toLocaleString("ja-JP")}</span></b>`;
  };
  ...
  function moneyRecord(){
    ...
    return `<section class="v2-page v2-money-record">${top(titleFor(page))}<div class="v2-balance"><div class="v2-balance-row primary"><span>今日使えるお金</span><button class="v2-privacy" data-v2-private>${S.ui.moneyVisible ? icon("eyeoff") + "隠す" : icon("eye") + "表示"}</button>${privateRecordMoney(c.left)}</div><div class="v2-balance-row"><span>今残っているお金</span>${privateRecordMoney(remaining)}</div></div><div class="v2-tabs"><button class="${moneyType === "expense" ? "on expense" : ""}" data-v2-moneytype="expense">支出</button><button class="${moneyType === "income" ? "on income" : ""}" data-v2-moneytype="income">収入</button></div><div class="v2-form-row"><label for="v2Amount">金額</label><input id="v2Amount" type="text" inputmode="numeric" placeholder="￥0"></div><div class="v2-form-row"><label for="v2Method">${moneyType === "income" ? "受け取り方法" : "支払い方法"}</label><select id="v2Method">${options.methods.map(([v,l])=>`<option value="${esc2(v)}" ${v===currentMethod?"selected":""}>${esc2(l)}</option>`).join("")}</select></div><div class="v2-form-row"><label for="v2Category">${moneyType === "income" ? "収入のカテゴリー" : "支出のカテゴリー"}</label><select id="v2Category">${options.cats.map(x=>`<option>${esc2(x)}</option>`).join("")}</select></div><button class="v2-primary ${moneyType === "income" ? "income" : ""}" data-v2-money-save>${recordTitle}</button><button class="v2-sub-action" data-v2-go="moneyOutlook">${icon("wallet")}<span>お金の見通しを見る</span></button><div class="v2-activity">${entries.length ? entries.map(x=>`<div class="v2-activity-row ${x.type}"><span><i class="v2-category-dot ${categoryTone(x.cat)}"></i>${esc2(x.cat)}</span><b>${x.type === "income" ? "+" : ""}${money(x.amt)}</b></div>`).join("") : `<p class="v2-empty">今日はまだ記録がありません</p>`}</div></section>`;
  }
