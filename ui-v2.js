...
(() => {
  ...
  const icon = (name, cls = "") => {
    const approved = {
      ...
      settings:'<path d="M4 6.5h16M4 12h16M4 17.5h16"/><circle cx="9" cy="6.5" r="2"/><circle cx="15" cy="12" r="2"/><circle cx="11" cy="17.5" r="2"/>',
      ...
      money:'<circle cx="12" cy="12" r="8.5"/><path d="m7.5 6.5 4.5 5.5 4.5-5.5M12 12v6M8.5 14h7M8.5 16.5h7"/>',
  ...
  function branch(kind){
    ...
    const question = kind === "record" ? "何を記録しますか？" : kind === "today" ? "何をしますか？" : "何を見える化しますか？";
    return `<section class="v2-page v2-branch">${top(titleFor(kind),true)}<div class="v2-branch-intro"><p class="v2-kicker">${icon(kind === "record" ? "edit" : kind === "today" ? "sun" : "chart")}${kind === "record" ? "RECORD" : kind === "today" ? "TODAY" : "REVIEW"}</p><h2>${question}</h2></div><div class="v2-list">${data.map(x=>choice(...x)).join("")}</div></section>`;
  }
  ...
  function healthRecord(){
    ...
    return `<section class="v2-page v2-health-record">${top(titleFor(page))}<p class="v2-kicker">${icon("heart")}HEALTH LOG</p><h2 class="v2-page-lead">今日のこころとからだ</h2><div class="v2-rate-section"><h2>${icon("body")}からだの調子</h2>${dots("body",h.body)}</div><div class="v2-rate-section mind"><h2>${icon("heart")}こころの調子</h2>${dots("mind",h.mind)}</div><div class="v2-health-metrics"><div><span>${icon("moon")}睡眠時間</span><strong>${fmtSleep(sm)}</strong></div><div><span>${icon("foot")}歩数</span><strong>${steps}</strong></div></div><div class="v2-form-heading v2-health-time-heading">${icon("clock")}睡眠の記録</div><div class="v2-form-row"><label for="v2Bed">${icon("moon")}就寝時刻</label><input id="v2Bed" type="time" value="${esc2(h.bed||"")}" data-v2-health="bed"></div><div class="v2-form-row"><label for="v2Wake">${icon("sun")}起床時刻</label><input id="v2Wake" type="time" value="${esc2(h.wake||"")}" data-v2-health="wake"></div><button class="v2-primary income" data-v2-health-save>${icon("download")}この日の記録を保存</button><button class="v2-sub-action" data-v2-go="healthAnalysis">${icon("chart")}体調を分析する</button></section>`;
  }
  ...
  function homeV2(){
    return `<section class="v2-page v2-home"><div class="v2-home-top"><span class="v2-home-logo"><span class="v2-mark"><i></i><i></i><i></i></span><strong>LIFE NOTE</strong></span><button aria-label="設定" data-v2-go="settings">${icon("settings")}</button></div><p class="v2-home-tagline">暮らしをまとめて管理するノート</p><h1 class="v2-home-title">今日は、何をしますか？</h1><div class="v2-list">${choice("record","money","edit","記録する",null,"01")}${choice("today","today","sun","今日を整える",null,"02")}${choice("visualize","analysis","chart","見える化する",null,"03")}</div><p class="v2-home-foot">必要なところから、ひとつずつ。</p></section>`;
  }
